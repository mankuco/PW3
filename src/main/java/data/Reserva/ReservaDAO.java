...
public class ReservaDAO extends DAO {
	...
	public ArrayList<ReservaDTO> verReservas() {
		...
		try {
			...
			while(rs.next()) {
				...
				reserva = new ReservaDTO(idReserva, idUsuario, minutosReserva, idPista, precioPista, tipoReserva, modalidad, fecha, hora, 0, numeroNinos, numeroAdultos);
	...
	public void modificarReserva(ReservaDTO r) {
		try{
			Connection con = getConnection();
			PreparedStatement ps = con.prepareStatement("UPDATE Reservas SET idUsuario = ?, minutos= ? ,precio= ?, modalidad= ?, idPista= ?, NumeroNinos= ?, NumeroAdultos= ?, fecha= ?, hora= ?, tipo= ? WHERE id = ?", PreparedStatement.RETURN_GENERATED_KEYS);
			ps.setString(1,r.getIdUsuario());
			ps.setInt(2,r.getMinutosReserva());
			ps.setFloat(3, r.getPrecioPista());
			ps.setString(4, r.getModalidad());
			ps.setInt(5, r.getIdPista());
			ps.setInt(6,r.getNumeroNinos());
			ps.setInt(7, r.getNumeroAdultos());
